...
if __name__ == "__main__":
    print("instantiating object before establishing relationship")
    manager = DigitalSafeManager("Safe manager - OYALES")
    
    # 3 Digitalsafe objects with varying storage and compartment counts
    ...
    safe3 = Digitalsafe("Backup Vault", 1000, "sysadmin", "root4321", compartments=25)

    print("\n INITIAL STATE ")
    manager.show_all_managed_safes()

    print("\n---RELATIONSHIP---")
    manager.register_safe(safe1)
    manager.register_safe(safe2)
    manager.register_safe(safe3)

    print("\n--- PERFORMING ACTIONS ON INDIVIDUAL SAFES ---")
    safe1.addfile("passwords.txt")
    safe2.addfile("tax_2026.pdf")
